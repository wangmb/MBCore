...
open class MBRouter:NSObject{
    ...
    /**
     ...
     - returns: AnyObject
     */
    internal func routeURL(url:NSURL,completion:NSDictionary) throws -> AnyObject?{
    ...
    /**
     ...
     - returns: AnyObject
     */
    private func routeAction(targetName:String,actionName:String,params:NSDictionary?=nil) throws -> AnyObject?{
        ...
        guard let targetClass = NSClassFromString((clsName as! String)+"."+targetClassStr) as? NSObject.Type else {
            DDLogError("\(clsName)中\(targetClassStr)类不存在")
        ...
        if target.responds(to: action){//判断调用方法是否存在
            DDLogVerbose("调用\(targetClassStr)类中\(actionStr)方法")

            return target.perform(action, with: params).takeUnretainedValue()
        }else{//调用方法不存在
            DDLogError("\(targetClassStr)类中不存在\(actionStr)方法")
